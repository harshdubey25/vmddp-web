...
export default function SchemeCard({ icon: Icon, title, description, benefits, componentId, index, isInView }: SchemeCardProps) {
  return (
    ...
    >
      <Card className="hover-elevate transition-all duration-300 h-full" data-testid={`card-scheme-${componentId}`}>
        <CardHeader>
          <motion.div
        ...
        <CardContent>
          ...
          <Link href={`/schemes/${componentId}`}>
            <Button variant="outline" className="w-full group" data-testid={`button-learn-more-${componentId}`}>
              Learn More
