                                            ...
                                            <TableRow key={payment.name} data-testid={`row-payment-${payment.name}`}>
                                                ...
                                                <TableCell>
                                                    <div className="space-y-1 max-w-xs">
                                                        {payment.component_allocations && payment.component_allocations.length > 0 ? (
                                                            <>
                                                                <div className="flex items-center gap-1 mb-1">
                                                                    <Badge variant="outline" className="bg-blue-500/10 text-blue-600">
                                                                        {payment.component_allocations.length} allocations
                                                                    </Badge>
                                                                </div>
                                                                <div className="space-y-0.5">
                                                                    {payment.component_allocations.map((alloc, idx) => (
                                                                        <div key={idx} className="text-xs">
                                                                            <span className="font-medium">{alloc.component_allocation}</span>
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                            </>
