...
import { Search, Download, Eye, FileText, FileSpreadsheet } from "lucide-react";
...
function buildExportParams(status: string, search: string, component: string, startDate: string, endDate: string) {
...
function buildExportFilename(prefix: string, status: string, search: string, component: string) {
    const parts: string[] = [];
    ...
    if (component !== "all") {
        parts.push(component.replace(/\s+/g, "_"));
    }
...
export default function SubAdminApplications() {
    ...
    const handleExport = useCallback((format: "excel" | "pdf") => {
        ...
        void exportApplications({
            format,
            filename: buildExportFilename("applications-page", statusFilter, searchQuery, componentFilter),
            params: {
                ...buildExportParams(statusFilter, searchQuery, componentFilter, dateFrom, dateTo),
                page: currentPage.toString(),
                limit: pageSize.toString(),
            },
        });
    }, [applications.length, componentFilter, currentPage, dateFrom, dateTo, exportApplications, pageSize, searchQuery, statusFilter, toast]);
    ...
    const handleExportAll = useCallback((format: "excel" | "pdf") => {
        ...
        void exportApplications({
            format,
            filename: buildExportFilename("applications", statusFilter, searchQuery, componentFilter),
            params: buildExportParams(statusFilter, searchQuery, componentFilter, dateFrom, dateTo),
        });
    }, [componentFilter, dateFrom, dateTo, exportApplications, pagination?.total_applications, searchQuery, statusFilter, toast]);
                ...
                <div className="flex gap-2">
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="gap-1 sm:gap-2 text-xs sm:text-sm h-8 sm:h-10" data-testid="button-export" disabled={isExporting}>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="default" className="gap-1 sm:gap-2 text-xs sm:text-sm h-8 sm:h-10" data-testid="button-export-all" disabled={isExporting}>
                                <Download className="w-3 h-3 sm:w-4 sm:h-4" />
